...
def seance_1(ac=dyn.Param_A319()):
    ...
    plot_dphr(ac,f'./plots/{ac.get_name()}_dphr.png')
    pass

def dphr(P,alpha):return-(P.Cm0 - P.ms*P.CLa*(alpha-P.a0))/P.Cmd

def plot_dphr(P, filename=None):
    alphas = np.deg2rad(np.linspace(-10, 20, 30))
    mss = [-0.1, 0., 0.2, 1.]
    ...
    for ms in mss:
        P.set_mass_and_static_margin(0.5, ms)
        plt.plot(np.rad2deg(alphas),dphr(P,alphas))
    ut.decorate(plt.gca(), u'dphrEq {}'.format(P.name), r'$\alpha$ en degres', '$dphrEq$',
                ['$ms =  ${: .1f}'.format(ms) for ms in mss])
    ut.savefig(filename)
